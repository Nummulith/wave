...
и экспортирует ноты для диапазона MIDI 12..47 (0..2 октавы) двумя способами:
  1) FFT-based harmonic template (interpolated + smoothing + anti-alias)
...
N_HARM = 32           # максимум гармоник для анализа (на синтезе отрежем по Nyquist)
MIDI_MIN = 12         # C0-ish
...
def estimate_adsr(y, sr=SAMPLE_RATE):
    ...
    # decay: find first frame after peak below some fraction (e.g. 0.7)
    target = 0.7
    ...
    # sustain: median after attack+decay until near-end
    ...
    # release: find tail where RMS falls below small fraction of sustain
    rev = rms[::-1]
...
def extract_template_fft(wav_path, f0_hz, n_harm=N_HARM, sr=SAMPLE_RATE):
    ...
    # prefilter input to avoid analyzing extreme HF artefacts
    ...
    # noise estimate — residual energy outside harmonic bands
...
def extract_template_lpc(wav_path, f0_hz, n_harm=N_HARM, sr=SAMPLE_RATE, lpc_order=32):
    y, sr_loaded = librosa.load(wav_path, sr=sr)
    # take a centered window to get steady portion
    N = len(y)
    center = N//2
    ...
    # sample envelope at harmonic frequencies
    amps = np.zeros(n_harm, dtype=np.float32)
    for k in range(1, n_harm+1):
        f_h = f0_hz * k
        if f_h >= sr_loaded/2:
        ...
        # average a small neighbourhood on the envelope to be robust
    ...
    # noise: estimate ratio of energy not captured by envelope on the whole spectrum
    # reconstruct harmonic energy approximation on original spectrum
    # (rough heuristic) — we set small noise since LPC tries to model spectral envelope
    noise = 0.03
